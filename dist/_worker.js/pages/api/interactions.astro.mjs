...
import { k as getAugmentedNamespace, l as getDefaultExportFromCjs } from '../../chunks/astro/server_DIId5cqJ.mjs';
...
const EPHEMERAL_FLAG$1 = 1 << 6;

const EMO = { sun: '🌟', die: '🎲', fear: '🫧', hope: '✨', crit: '🎉' };
...
function buildMessage({ content = null, embeds = [], ephemeral = false }) {
  ...
  if (ephemeral) data.flags = EPHEMERAL_FLAG$1;
  return { type: 4, data };
}

async function handle$2(interaction) {
...
async function handle$1(interaction, env, ctx) {
...
async function handle(interaction, env, ctx) {
    return {
        type: 4,
        data: {
